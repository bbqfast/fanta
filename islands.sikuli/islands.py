...
{"num":2,"loc":Location(920, 188), "user":"bb9now2", "pass":"yyyyyyyy", "reboot":False},
...
def getGame(info):
    gid=info["num"]
    print("Activate user=" + info["user"] + " gid=" + str(gid))
...
def handleDisconnect(info):
    ...
    #playIfexists(game)
    if game.exists(imgIslands, existsTimeout):
        startGame(game, info)
    log("check disconnect")
    if game.exists(imgDis, existsTimeout):
        clickWithCatch(game, imgDis)
        closeGame()
        startGame(game, info)
        log("game restarted")
    log("check tele error")
    # imgConnError
    if game.exists(imgTeleportError, existsTimeout):
        clickWithCatch(game, imgTeleportError)
        closeGame()
        startGame(game, info)
        log("game restarted")
    log("check login button")
    if game.exists(imgLoginBtn, existsTimeout):
        clickWithCatch(game, imgLoginBtn)
        loginIfNeeded(game, info)
        sleep(10)
        startGame(game, info)
    log("check login textbox")
    if game.exists(imgLoginText, existsTimeout):
